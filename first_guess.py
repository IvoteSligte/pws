# pws_rust contains a parallelized version of this which runs several hundred times faster.
...
for i, a in enumerate(allowed_wordle_words):
    ...
    time_left = (len(allowed_wordle_words) - (i + 1)) * (time_taken / (i + 1))
    print(f"{i} / {len(allowed_wordle_words)} | Time taken: {format_secs(time_taken)} | Estimated time left: {format_secs(time_left)}")
